...
describe('SearchForms Component', () => {
  ...
  test('обработка поиска по названию', async () => {
    ...
    const { getByPlaceholderText, getByText } = render(
      <SearchForms onSearchResults={onSearchResults} />,
    );

    fireEvent.change(getByPlaceholderText('Введите название'), {
      target: { value: 'Тестовый' },
    });
    fireEvent.click(getByText('Поиск по названию'));

    await waitFor(() =>
      expect(fetchMoviesByTitle).toHaveBeenCalledWith('Тестовый'),
    );
  ...
  test('сброс фильтров работает корректно', async () => {
    const { getByText, getByPlaceholderText } = render(
      <SearchForms onSearchResults={() => {}} />,
    );

    fireEvent.change(getByPlaceholderText('Рейтинг от'), {
      target: { value: '8' },
    });
    ...
    await waitFor(() =>
      expect(
        (getByPlaceholderText('Рейтинг от') as HTMLInputElement).value,
      ).toBe(''),
    );
  });
